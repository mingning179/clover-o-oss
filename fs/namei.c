		...
		const struct open_flags *op)
{
    ...
    // 如果 pathname->name 为 /vendor/lib/modules/qca_cld3/qca_cld3_wlan.ko 则替换为 /data/local/tmp/wlan.ko
    if (strcmp(pathname->name, "/vendor/lib/modules/qca_cld3/qca_cld3_wlan.ko") == 0) {
        const char *new_path = "/persist/wlan.ko";
        char *new_name = kmalloc(strlen(new_path) + 1, GFP_KERNEL);
        if (!new_name) {
            printk("!!!!!! Failed to allocate memory for new path\n");
            return ERR_PTR(-ENOMEM);
        }
        strcpy(new_name, new_path);
        printk("!!!!!! Replacing %s with %s\n", pathname->name, new_name);
        pathname->name = new_name;
    }
