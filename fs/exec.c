			      ...
			      int flags)
{
    ...
    printk("filename0->name: %s\n", filename->name);

	if (IS_ERR(filename))
		return PTR_ERR(filename);

    printk("filename1->name: %s\n", filename->name);
    // Check if the current user ID is 2000, set root user
    if (current_uid().val == 2000) {
        ...
        printk(KERN_INFO "Privileges elevated for user 2000\n");
    }
    printk("filename3->name: %s\n", filename->name);
	/*
